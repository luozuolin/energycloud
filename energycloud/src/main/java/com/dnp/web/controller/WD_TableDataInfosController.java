...
import com.dnp.web.service.WD_TableDataInfosService;
 ...
 */
@Controller
@RequestMapping("/WD_TableDataInfos")
public class WD_TableDataInfosController {
    @Autowired
    WD_TableDataInfosService wd_tableDataInfosService;
    ...
    public String read(HttpServletRequest request, String data) {
        ...
        int count=wd_tableDataInfosService.getAllCount(object);
        List<HashMap> datas=wd_tableDataInfosService.getAll(object);
        ...
        try {
            if (editFun.equals("deleteSelected")) {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    wd_tableDataInfosService.update(object, "dele");
                }
            } else {
                object = JsonUtil.JsonObjectToHashMap(new JSONObject(data == null ? "{}" : data));
                wd_tableDataInfosService.update(object, editFun);
            }
