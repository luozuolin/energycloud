...
import com.dnp.web.service.WD_CircuitRefGroupInfosService;
...
*/
@Controller
@RequestMapping("/wd_circuitRefGroup")
public class WD_CircuitRefGroupInfosController {
    @Autowired
    WD_CircuitRefGroupInfosService wd_circuitRefGroupInfosService;
    ...
    public String read(HttpServletRequest request, String data) {
        ...
        int count= wd_circuitRefGroupInfosService.getAllCount(object);
        List<HashMap> datas=wd_circuitRefGroupInfosService.getAll(object);
        ...
        try {
            if (editFun.equals("deleteSelected")) {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    wd_circuitRefGroupInfosService.update(object, "dele");
                }
            } else {
                object = JsonUtil.JsonObjectToHashMap(new JSONObject(data == null ? "{}" : data));
                wd_circuitRefGroupInfosService.update(object, editFun);
            }
