...
import com.dnp.web.service.WD_UserDeviceInfosService;
 ...
 */
@Controller
@RequestMapping("/wd_userDevice")
public class WD_UserDeviceInfosController {
    @Autowired
    WD_UserDeviceInfosService wd_userDeviceInfosService;
    ...
    public String read(HttpServletRequest request, String data) {
        ...
        int count= wd_userDeviceInfosService.getAllCount(object);
        List<HashMap> datas=wd_userDeviceInfosService.getAll(object);
        ...
        try {
            if (editFun.equals("deleteSelected")) {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    wd_userDeviceInfosService.update(object, "dele");
                }
            } else {
                object = JsonUtil.JsonObjectToHashMap(new JSONObject(data == null ? "{}" : data));
                wd_userDeviceInfosService.update(object, editFun);
            }
    ...
    public String read(HttpServletRequest request) {
        List<HashMap> datas=wd_userDeviceInfosService.listAll();
