...
import com.dnp.web.service.WD_DataUGRefCGService;
import com.dnp.web.service.WD_DataUserGroupService;
 ...
 */
@Controller
@RequestMapping("/WD_DataUserGroup")
public class WD_DataUserGroupController {
    @Autowired
    WD_DataUserGroupService wd_dataUserGroupService;
    @Autowired
    WD_UserRefDataUGService wd_userRefDataUGService;
    @Autowired
    WD_DataUGRefCGService wd_dataUGRefCGService;
    ...
    public String read(HttpServletRequest request, String data) {
        ...
        int count=wd_dataUserGroupService.getAllCount(object);
        List<HashMap> datas=wd_dataUserGroupService.getAll(object);
        ...
        try {
            if (editFun.equals("deleteSelected")) {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    wd_dataUserGroupService.update(object, "dele");
                }
            } else {
                object = JsonUtil.JsonObjectToHashMap(new JSONObject(data == null ? "{}" : data));
                wd_dataUserGroupService.update(object, editFun);
            }
    ...
    public String listAll(HttpServletRequest request, String data) {
        List<HashMap> datas=wd_dataUserGroupService.listAll();
        return JsonUtil.toJson(datas);
    }
    @RequestMapping(value = "/getCircuitGroupBynumDataUserGroupID", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
    @ResponseBody
    public String getCircuitGroupBynumDataUserGroupID(HttpServletRequest request,String data) {
        HashMap<String,Object> object = JsonUtil.JsonObjectToHashMap(new JSONObject(data==null?"{}":data));
        List<HashMap> datas=wd_dataUserGroupService.getCircuitGroupBynumDataUserGroupID(object);
        return JsonUtil.toJson(datas);
    }
    @RequestMapping(value = "/getUserBynumDataUserGroupID", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
    @ResponseBody
    public String getUserBynumDataUserGroupID(HttpServletRequest request,String data) {
        HashMap<String,Object> object = JsonUtil.JsonObjectToHashMap(new JSONObject(data==null?"{}":data));
        List<HashMap> datas=wd_dataUserGroupService.getUserBynumDataUserGroupID(object);
        return JsonUtil.toJson(datas);
    }

  //  SELECT  * from WD_DataUGRefCG;
 //   SELECT   * from WD_UserRefDataUG;
    ...
    public String updateFuncUG(HttpServletRequest request, String data) {
        ...
        wd_dataUGRefCGService.deleteBynumDataUserGroupId(object);
        String[] ids=object.get("ids").toString().split(",");
        for(int i=0;i<ids.length;i++)
        {
            object.put("numCircuitGroupID",ids[i]);
            wd_dataUGRefCGService.insert(object);
        }
        return   null;
    }
    ...
    public String updateDataUG(HttpServletRequest request, String data) {
        HashMap<String,Object> object = JsonUtil.JsonObjectToHashMap(new JSONObject(data==null?"{}":data));
        //首先删除WD_FuncUGRefFunc中 numFunctionUserGroupID的数据，接着插入data中的功能数据
        wd_userRefDataUGService.deleteBynumDataUserGroupId(object);
        String[] ids=object.get("ids").toString().split(",");
        for(int i=0;i<ids.length;i++)
        {
