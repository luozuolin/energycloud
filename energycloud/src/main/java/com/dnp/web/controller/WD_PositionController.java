...
import com.dnp.web.service.WD_PositionService;
 ...
 */
@Controller
@RequestMapping("/WD_Position")
public class WD_PositionController {
    @Autowired
    WD_PositionService wd_positionService;

//    @RequestMapping(value = "/getAll", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
//    @ResponseBody
//    public String read(HttpServletRequest request) {
//        List<HashMap> datas=wd_positionService.getAll();
//        return JsonUtil.toJson(datas);
//    }
//    @RequestMapping(value="",method = {RequestMethod.GET})
//    public ModelAndView index(ModelAndView mv){
//        mv.setViewName("/Purview/PositionInfos");
//        return mv;
//    }
    ...
    public String read(HttpServletRequest request, String data) {
        ...
        int count=wd_positionService.getAllCount(object);
        List<HashMap> datas=wd_positionService.getAll(object);
        ...
        try {
            if (editFun.equals("deleteSelected")) {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    wd_positionService.update(object, "dele");
                }
            } else {
                object = JsonUtil.JsonObjectToHashMap(new JSONObject(data == null ? "{}" : data));
                wd_positionService.update(object, editFun);
            }
    ...
    @ResponseBody
    public String read(HttpServletRequest request) {
        List<HashMap> datas=wd_positionService.listAll();
        return JsonUtil.toJson(datas);
    }
