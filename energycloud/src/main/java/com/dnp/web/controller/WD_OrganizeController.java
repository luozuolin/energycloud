 ...
 */
@Controller
@RequestMapping("/WD_Organize")
public class WD_OrganizeController {
    @Autowired
    com.dnp.web.service.WD_OrganizeService WD_OrganizeService;
    ...
    public String read(HttpServletRequest request) {
        List<HashMap> datas=WD_OrganizeService.listAll();
        return JsonUtil.toJson(datas);
    }
    @RequestMapping(value = "/OrganizelistAll", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
    @ResponseBody
    public String OrganizelistAll(HttpServletRequest request) {
        List<HashMap> datas=WD_OrganizeService.OrganizelistAll();
        return JsonUtil.toJson(datas);
    }
    @RequestMapping(value = "/getBynumUserID", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
    @ResponseBody
    public String getBynumUserID(HttpServletRequest request,String data) {
        HashMap<String,Object> object = JsonUtil.JsonObjectToHashMap(new JSONObject(data==null?"{}":data));
        List<HashMap> datas=WD_OrganizeService.getBynumUserID(object);
        return JsonUtil.toJson(datas);
    }
