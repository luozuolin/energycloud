...
import com.dnp.web.service.WD_CircuitParentsInfosService;
...
/**
* @Auther Tao 
* @date:2016/9/26 14:14
*/
@Controller
@RequestMapping("/wd_circuitParents")
public class WD_CircuitParentsInfosController {
    @Autowired
    WD_CircuitParentsInfosService wd_circuitParentsInfosService;
    ...
    public String read(HttpServletRequest request, String data) {
        ...
        int count=wd_circuitParentsInfosService.getAllCount(object);
        List<HashMap> datas=wd_circuitParentsInfosService.getAll(object);
        ...
        try {
            if (editFun.equals("deleteSelected")) {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    wd_circuitParentsInfosService.update(object, "dele");
                }
            } else {
                object = JsonUtil.JsonObjectToHashMap(new JSONObject(data == null ? "{}" : data));
                wd_circuitParentsInfosService.update(object, editFun);
            }
