 ...
 */
@Controller
@RequestMapping("/WD_UserRefFuncUG")
public class WD_UserRefFuncUGController {
    @Autowired
    ...
    public String read(HttpServletRequest request, String data) {
        ...
        int count=wd_userRefFuncUGService.getAllCount(object);
        List<HashMap> datas=wd_userRefFuncUGService.getAll(object);
        ...
        try {
            if (editFun.equals("deleteSelected")) {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    wd_userRefFuncUGService.update(object, "dele");
                }
            } else {
                object = JsonUtil.JsonObjectToHashMap(new JSONObject(data == null ? "{}" : data));
                wd_userRefFuncUGService.update(object, editFun);
            }
