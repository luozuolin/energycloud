...
import com.dnp.web.service.DataErrorInfosService;
 ...
 */
@Controller
@RequestMapping("/WD_DataError")
public class DataErrorInfosController {
    @Autowired
    DataErrorInfosService dataErrorInfosService;
    ...
    public String read(HttpServletRequest request, String data) {
        ...
        int count=dataErrorInfosService.getAllCount(object);
        List<HashMap> datas=dataErrorInfosService.getAll(object);
        ...
        try {
            if (editFun.equals("deleteSelected")) {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    dataErrorInfosService.update(object, "dele");
                }
            } else {
                object = JsonUtil.JsonObjectToHashMap(new JSONObject(data == null ? "{}" : data));
                dataErrorInfosService.update(object, editFun);
            }
