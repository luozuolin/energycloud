 ...
 */
@Controller
@RequestMapping("/WD_FuncUGRefFunc")
public class WD_FuncUGRefFuncController {
    ...
    public String read(HttpServletRequest request, String data) {
        ...
        int count=wd_funcUGRefFuncService.getAllCount(object);
        List<HashMap> datas=wd_funcUGRefFuncService.getAll(object);
        ...
        try {
            if (editFun.equals("deleteSelected")) {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    wd_funcUGRefFuncService.update(object, "dele");
                }
            } else {
                object = JsonUtil.JsonObjectToHashMap(new JSONObject(data == null ? "{}" : data));
                wd_funcUGRefFuncService.update(object, editFun);
            }
