...
import com.dnp.web.service.WD_TableRefCircuitInfosService;
 ...
 */
@Controller
@RequestMapping("/wd_tableRefCircuit")
public class WD_TableRefCircuitInfosController {
    @Autowired
    WD_TableRefCircuitInfosService wd_tableRefCircuit;
    ...
    public String read(HttpServletRequest request, String data) {
        ...
        int count=wd_tableRefCircuit.getAllCount(object);
        List<HashMap> datas=wd_tableRefCircuit.getAll(object);
        ...
        try {
            if (editFun.equals("deleteSelected")) {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    wd_tableRefCircuit.update(object, "dele");
                }
            } else {
                object = JsonUtil.JsonObjectToHashMap(new JSONObject(data == null ? "{}" : data));
                wd_tableRefCircuit.update(object, editFun);
            }
