...
import com.dnp.web.service.WD_FunctionService;
 ...
 */
@Controller
@RequestMapping("/WD_Function")
public class WD_FunctionController {
    @Autowired
    WD_FunctionService wd_functionService;
    ...
    public String read(HttpServletRequest request, String data) {
        ...
        int count=wd_functionService.getAllCount(object);
        List<HashMap> datas=wd_functionService.getAll(object);
        ...
        try {
            if (editFun.equals("deleteSelected")) {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    wd_functionService.update(object, "dele");
                }
            } else {
                object = JsonUtil.JsonObjectToHashMap(new JSONObject(data == null ? "{}" : data));
                wd_functionService.update(object, editFun);
            }
    ...
    public String read(HttpServletRequest request) {
        List<HashMap> datas=wd_functionService.listAll();
        return JsonUtil.toJson(datas);
    }
    @RequestMapping(value = "/getFunctionUserGroupBynumFunctionID", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
    @ResponseBody
    public String getFunctionUserGroupBynumFunctionID(HttpServletRequest request,String data) {
        HashMap<String,Object> object = JsonUtil.JsonObjectToHashMap(new JSONObject(data==null?"{}":data));
        List<HashMap> datas=wd_functionService.getFunctionUserGroupBynumFunctionID(object);
        return JsonUtil.toJson(datas);
    }
 //   updateFuncUG
    ...
    public String setFunc(HttpServletRequest request, String data) {
        ...
        wd_funcUGRefFuncService.deleteBynnumFunctionID(object);
